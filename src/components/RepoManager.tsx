...
export const RepoManager = ({ children }: PropsWithChildren) => {
  ...
  const fetchRepos = async () => {
    ...
    async function fetchReposFromGitHub(): Promise<RepositoryNode[]> {
      const headers = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${process.env.GITHUB_TOKEN}`,
      };
    ...
    `;
      ...
      const [userRepositories, pinnedRepos, orgContributions] =
        await Promise.all([
      ...
      const { pinnedItems }: { pinnedItems: PinnedItems } =
        pinnedRepos.data.user;
